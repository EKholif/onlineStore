...
public class ProductController {
    ...
    public ModelAndView viewProductDetail(@PathVariable("product_alias") String alias, HttpServletRequest request)
        ...
        if (product != null && product.getId() != null) {
            ...
            // Assuming Filter sets it.
            if (tenantIdLong != null) {
                Integer tenantId = tenantIdLong.intValue();
                eventPublisher.publishEvent(new com.onlineStoreCom.entity.analytics.event.ProductViewEvent(
    ...
    public ModelAndView listByPage(@PathVariable(name = "pageNum") int pageNum,
        ...
        // [AG-PSI-002] Publish Search Event (Async Analytics)
        if (keyWord != null && !keyWord.trim().isEmpty()) {
            Long tenantIdLong = com.onlineStoreCom.tenant.TenantContext.getTenantId();
            if (tenantIdLong != null) {
                eventPublisher.publishEvent(
                        new com.onlineStoreCom.entity.analytics.event.SearchEvent(keyWord, tenantIdLong.intValue()));
            }
        }
