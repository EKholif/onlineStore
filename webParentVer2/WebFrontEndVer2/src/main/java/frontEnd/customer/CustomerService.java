...
public class CustomerService {
    ...
    public boolean isEmailUnique(Integer id, String email) {
        // AG-TEN-FIX: Check uniqueness within current tenant scope
        Long tenantId = com.onlineStoreCom.tenant.TenantContext.getTenantId();
        Customer customerByEmail = customerRepository.findByEmailAndTenantId(email, tenantId);

        boolean isCreatingNew = (id == null);

        if (isCreatingNew) {
            // في حالة إنشاء عميل جديد، الإيميل لازم يكون مش موجود في نفس الـ Tenant
    ...
    public Customer saveCustomer(Customer customer) {
        ...
        if (existingCustomer) {
            // Retrieve existing data by ID to handle detached entity correctly
            Customer existingInDb = customerRepository.findById(customer.getId()).orElse(null);

            if (existingInDb != null) {
                // Secure Password Preservation Logic
                // If password in form is null or empty/whitespace, keep the old one
                if (customer.getPassword() == null || customer.getPassword().trim().isEmpty()) {
                    customer.setPassword(existingInDb.getPassword());
                } else {
                    encodePassword(customer);
                }

                // Preserve critical system fields that form might lose or reset
                customer.setCreatedTime(existingInDb.getCreatedTime());
                customer.setVerificationCode(existingInDb.getVerificationCode());
                customer.setAuthenticationType(existingInDb.getAuthenticationType());
                customer.setRestPasswordToken(existingInDb.getRestPasswordToken());
                
                // Keep enabled status unless explicitly handled (safer than forcing true)
                customer.setEnabled(existingInDb.isEnabled());
            }
    ...
    public String updaterest_password(String email) throws CustomerNotFoundException {
        // AG-TEN-FIX: Use tenant-scoped query for password reset
        Long tenantId = com.onlineStoreCom.tenant.TenantContext.getTenantId();
    ...
    /**
     * AG-TEN-FIX: Get customer by email within current tenant scope
     * This prevents NonUniqueResultException when same email exists in multiple tenants
     */
