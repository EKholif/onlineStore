...
public interface SettingRepository extends JpaRepository<Setting, String> {
    ...
    // Query Level
    @Query("SELECT s FROM Setting s WHERE s.category = ?1 AND (s.tenantId = :#{T(com.onlineStoreCom.tenant.TenantContext).getTenantId()} OR s.tenantId = 0 OR s.tenantId IS NULL) ORDER BY s.tenantId ASC")
    ...
    @Query("SELECT s FROM Setting s WHERE s.key = ?1 AND (s.tenantId = :#{T(com.onlineStoreCom.tenant.TenantContext).getTenantId()} OR s.tenantId = 0 OR s.tenantId IS NULL)")
