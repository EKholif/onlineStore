...
public class RewardService {
    ...
    public RewardSetting getSettings() {
        return rewardSettingRepo.findByTenant().orElseGet(() -> {
            // Should not happen if init is correct, but safe fallback
            RewardSetting s = new RewardSetting();
            // s.setTenantId(TenantContext.getTenantId()); // handled by listener usually
            return s;
        });
    }
    ...
    public boolean redeemPoints(Customer customer, int pointsToRedeem) {
        ...
        CustomerPoint history = new CustomerPoint(customer, -pointsToRedeem, PointType.CHECKOUT_REDEEM,
